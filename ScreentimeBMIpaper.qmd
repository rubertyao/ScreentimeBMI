...
## Hypotheses
 [@ma_effect_2024] make four hypotheses on what can increase body weight with internet use.
...
p1 = ggplot(bmi_data, aes(x = screeTIME, y = bmi)) +
  ...
  labs(title = "(a) BMI as explained by Screentime",
      x = "Screentime (minutes)", 
      y = "BMI")

p2 = ggplot(bmi_data, aes(x = sleep, y = bmi)) +
  geom_point(color = "darkblue") +
  labs(title = "(b) BMI as explained by Sleep Time",
      x = "Sleep Time (hours)", 
      y = "BMI")

p3 = ggplot(bmi_data, aes(x = t_exe, y = bmi)) +
  geom_point(color = "darkgreen") +
  labs(title = "(c) BMI as explained by Exercise",
      x = "Exercise Time (minutes)", 
      y = "BMI")

p4 = ggplot(bmi_data, aes(x = all_seditry, y = bmi)) +
  geom_point(color = "purple") +
  labs(title = "(d) BMI as explained by Sedentary Time",
...
$\beta_1$ represents the expected point change in BMI per point change in minutes spent on a screen a day holding other variables constant.
...
$X_{ji}$ represents the $i^{th}$ adolescent's $j^{th}$ predictor in the model.
Therefore, $X_{1i}$ is the $i^{th}$ adolescent's minutes in screen time, $X_{2i}$ is the $i^{th}$ adolescent's age, $X_{3i}$ is the $i^{th}$ adolescent's gender,and $X_{4i}$ is the $i^{th}$ adolescent's log household income.
...
$\beta_5$ represents the expected point change in BMI per point change in minutes spent exercising a day while holding other variables constant.

$\beta_6$ represents the expected point change in BMI per point change in spent sedentary a day while holding other variables constant.

$\beta_7$ represents the expected point change in BMI per point change in hours spent asleep a day while holding other variables constant.
...
We consider interactions between time spent on a screen with time spent exercising and time spent sleeping. We consider these interactions because Hypothesis 1 and 2 from [@ma_effect_2024] reason that higher screen time "crowds out" potential time spent sleeping and exercising, thereby reducing their respective totals and increasing weight gain.
...
Where $\beta_8$ represents the interaction effect between minutes spent on a screen and minutes spent exercising and $\beta_9$ represents the interaction effect between minutes spent on a screen and hours spent sleeping.
...
Where $Y_i$ is transformed, so each slope interpretation $\beta$ is modified to an expected inverse square root point change in BMI, holding all other variables constant.


## Ridge and LASSO Models 

We implement Ridge and Least Absolute Squares Selection Operator (LASSO) regression, both of which introduces a tuning parameter $\lambda$ that penalizes additional parameter terms. We select $\lambda$ and use k-fold cross validation with 10 folds. LASSO essentially performs variable selection by penalizing parameter values to 0.

For ridge regression, we select parameter values that minimize

$$\sum_{i=1}^n(\mathbf{Y}_i-\mathbf{X}_i^T\mathbf{\beta})^2+\lambda\sum_{j=1}^{p-1}\beta_j^2$$
...
where $\lambda$ is a tuning parameter. Since ridge regression found the best performing lambda value $=0$, we drop the model since it is statistically the same as the full model. LASSO found $\lambda$=.5.
```{r}
#| include: false
#| warning: false
#| message: false
...
k-fold Cross validation splits the data into k folds, and uses all but one fold of our training data to fit the model, and uses the final fold to test the model and make predictions. The process is then repeated for all folds and the root mean squared error across k folds calculated is as follows, where Y is the observed value and $\hat{y}$ is the predicted value: 
$$RMSE=\sqrt{\frac{1}{m}\sum_{j=1}^m(Y_{j}-\hat{Y}_{j})^2}$$
...
$$RMSE_{adj}=\sqrt{\frac{1}{m}\sum_{j=1}^m(Y_{j}^{-2}-\hat{Y}_{j}^{-2})^2}$$
...
rmse_models = sapply(allmodels, function(m) sqrt(mean((m$pred$obs-m$pred$pred)^2)))
rmsea_models = sapply(allmodels, function(m) sqrt(mean((m$pred$obs^(-2)-m$pred$pred^(-2))^2)))
...
sumtable = tibble(
  Model = c("Base Model","Full Model","Full Model with Interaction","Inverse Transformed Base Model","Inverse Transformed Full Model","Inverse Transformed Full Model with Interaction","Lasso Model","Inverse Transformed Lasso Model"),
  RMSE = rmse_models,
...
It is important to note that this survey data is not completely reliable. For example, the time in minutes recorded is most likely approximated and not the actual time spent doing various activities (eg., sleeping, screen time, physical activity). 
A null value majority in the smoking, drinking, and food consumption categories have prevented the use of those variables in the model, but can likely be imputed with estimates which fall outside of the scope of this analysis.
[ma_effect_2024] uses the full dataset, which includes data from 2005-2015. Since the scope of the analysis is only on 2015, a majority of the information in the dataset is lost. Additionally, no data dictionary has been supplied, therefore many of the variable values needed to be interpreted, since some of the merged variable names have been renamed by the study and definitions of them are not available on the Harvard Database or the CHS database.


## Software
 We implement the analysis using R [@r_language]. Plots done with [@ggplot] and [@patchwork]. Data manipulation done with [@tidyverse]. Boxcox transformations done with [@MASS]. Predictive modeling and cross validation done with [@caret]. Tables done with [@knitr]. 
...
The table of RMSE and $R^2$ values shows that the base model performed the best in terms of RMSE, and the inverse transformed base model performed the best in terms of $R^2$. The full model without any additional methods resulted in an RMSE of 4.4742 and an $R^2$ of 0.0916. Adding interaction terms to the full model increased RMSE by 0.0029 and decreased $R^2$ by 0.0011. Adding an inverse transformation to the base model increased RMSE by 0.0424 when adjusted for transformation for the base model. The LASSO model improved upon the full model, decreasing RMSE by 0.0457 and increasing $R^2$ by 0.0203.

## Final Model
We compare OLS vs Transformation vs LASSO while including interaction terms. As it turns out, the simplest model found the best root mean square error using cross validation, with RMSE 4.4212 and $R^2$ 0.11. Using other methods with the full model performed worse, with only slightly increased performance in $R^2$. It is important to note that the RMSE minimally changed between models, so each method only slightly changes performance. We choose the base model because it sacrifices the least in interpretability while also maintaining relatively similar performance. The estimated intercept term is 10.2, which has little interpretability, but is technically possible: a female child born within the year that has no screen time and no family income. The estimated slope parameter for screen time is -.0013, which represents an expected -0.0013 change in BMI for each additional point change in screen time, holding all other variables constant. The estimated slope parameter for age is 0.6395, meaning that for each point change in age, the expected increase in BMI is .6395, holding all other variables constant. The estimated slope parameter for gender is .7414, meaning that for a boy, the expected BMI is to be increased by .7414, holding all other variables constant. The estimated slope parameter for log income is 0.089, meaning that for each point increase in log income, BMI is expected to raise by 0.089, holding all other variables constant.
 
# Discussion {#sec-discussion}
Although we have selected a model that most accurately predicts bmi, since all of the $R^2$ values for our models were low, all models performed rather poorly. We conclude that for our analysis, we could not accurately predict BMI using screentime, age, sex, income, exercise time, sedentary time, sleep time, and age. Since the RMSE values were all greater than 4, and the BMI ranges for underweight, healthy, overweight, and obese are all around 6.5, our models have a strong likelihood of misclassifying, for example a healthy adolescent as overweight.
[@ma_effect_2024] used a binary variable as their main predictor in their empirical analysis, while the extension done by this paper uses a continuous variable, screen time. This distinction could explain the negative coefficient found for screen time, whereas in the previous study, internet use was found to significantly increase BMI.
...
One possible extension is the inclusion of Hypothesis 3 and 4 in @sec-data. We did not include these hypotheses in the model synthesizing process due to the large amount of N/A values in the rows, but with interpolation, further variable selection could be performed.
Since this analysis only included adolescents from 2015, the model could be extended towards children in the other years, controlling if a child was sampled in the previous years.
