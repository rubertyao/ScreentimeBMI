...
folds for each model. The model with the lowest RMSE was the LASSO model, but the final model chosen was the base model due to interpetation concerns.
...
For inference, initial visualizations of the errors showed no clear
...
The table of RMSE and $R^2$ values shows that the lasso model performed
the best in terms of RMSE. It is important to note that the $R^2$ for
the transformed models lack comparability to the non-transformed models,
so we compare the RMSE values for those instead. The full model without
any additional methods resulted in an RMSE of 4.4475 and an $R^2$ of
0.0792. Adding interaction terms to the full model decreased RMSE by
0.0224 and decreased $R^2$ by 0.0126. Adding an inverse transformation
to the base model increased RMSE by 0.0509 when adjusted for
transformation for the base model. The LASSO model improved upon the
full model with an RMSE of 4.3747 and $R^2$ of 0.1098.
...
terms. As it turns out, the LASSO model found the best root mean
square error using cross validation, with RMSE 4.3805 and $R^2$ 0.1117. However, as mentioned in @sec-methods, the algorithm chose a final model that only included age, which drops the main predictor we are interested in.
...
extended towards children in the other years, controlling if a child was
sampled in the previous years.
